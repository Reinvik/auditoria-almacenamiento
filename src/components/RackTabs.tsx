  ...
  return (
    <div className="bg-white border-b border-slate-200 px-4 sm:px-6 py-2.5 shadow-sm w-full">
      <div className="max-w-[1920px] mx-auto space-y-2.5">
        {/* Top-Level Section Navigation: Auditoría de Racks vs Reporte de Ocupación */}
        <div className="flex flex-wrap items-center justify-between gap-2.5 pb-2 border-b border-slate-200">
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl text-xs font-black flex items-center gap-2 transition-all cursor-pointer ${
                viewMode !== 'occupancy_report'
                  ? 'bg-[#0a5c36] text-white shadow-md shadow-emerald-950/20 ring-1 ring-[#08482a]'
                  : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
              }`}
            >
              <Columns className="w-4 h-4" />
              <span>📦 Auditoría de Racks (29 Racks)</span>
            </button>
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl text-xs font-black flex items-center gap-2 transition-all cursor-pointer ${
                viewMode === 'occupancy_report'
                  ? 'bg-[#0e4c68] text-white shadow-md shadow-sky-950/20 ring-2 ring-sky-300'
                  : 'bg-sky-50 text-[#0e4c68] hover:bg-sky-100 border border-sky-300 shadow-xs'
              }`}
            >
              <TrendingUp className="w-4 h-4 text-sky-600" />
              <span>📈 Reporte de Ocupación Frío (% Congelado / % Refrigerado)</span>
              <span className="px-1.5 py-0.2 rounded bg-sky-600 text-white text-[9.5px] font-black uppercase tracking-wider">
                Gráficos
              </span>
            </button>
          </div>
          ...
          {viewMode === 'occupancy_report' && (
            <button
              onClick={() => onChangeViewMode('audit_excel')}
              className="px-3 py-1 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-black flex items-center gap-1.5 transition-all cursor-pointer ml-auto border border-slate-300 shadow-xs"
            >
        ...
        {/* Solo mostrar controles de rack si NO estamos en el reporte global de ocupación */}
        {viewMode !== 'occupancy_report' && (
          <>
